...
#define LEVELS 3 /** Priority Levels */

/**
  @brief The Process-Thread control block

  An object of this type is associated to every thread and its process
  .In this object are stored all the metadata that are related to the thread.
*/

typedef struct pt_control_block{
  CondVar cv;     /*Condition variable to access this PTCB*/
  int exitval;    /*The exit value of the thread*/
  int exited;     /*If the thread is finished is 1 otherwise is 0*/
  Task task;      /*The program of the thread*/
  int argl;
  void* args;
  PCB* pcb;       /*The owner pcb of the thread*/
  TCB *tcb;       /*The tcb of the thread*/
  int joinable;   /*If the thread is joinable is 1 otherwise is 0*/
  Tid_t tid;      /*The id of the thread*/
  rlnode node;    /*The node of the PTCB*/
  int ref_counter;/*Count how many thread have accessed the ptcb*/ 
...
typedef struct thread_control_block
{
  ...
  PTCB* owner_ptcb;     /*The ptcb of the thread*/
  ...
  int priority; /**The scheduling priortiy of the thread */  
...
/* This function change the priority of the tcb. Set the "ud" to 0 for decresing
the priority of 1 for increasing */


/* */
void priority_booster(void);
